...
register_init_grad(_DictValues, lambda dvals: _OptionsAD(dvals))
...
def _translate_compute_source(comp, verbose=0):
    ...
    discrete = len(comp._discrete_inputs) > 0 or len(comp._discrete_outputs) > 0
...
def _dot_prod_test(comp, fwd_func, rev_func):
    ...
    discrete_inputs = comp._discrete_inputs
    discrete_outputs = comp._discrete_outputs
    discrete = len(discrete_inputs) > 0 or len(discrete_outputs) > 0
    ...
    try:
        ...
        if explicit:
            if discrete:
                fwd_grad = fwd_func(comp, inputs, outputs, discrete_inputs, discrete_outputs,
                                    dinputs)
            else:
                fwd_grad = fwd_func(comp, inputs, outputs, dinputs)
            doutputs._data[:] = rand_outputs
            if discrete:
                rev_grad = rev_func(comp, inputs, outputs, discrete_inputs, discrete_outputs,
                                    doutputs)
            else:
            ...
            doutputs._data[:] = rand_outputs
            if discrete:
                fwd_grad = fwd_func(comp, inputs, outputs, resids, discrete_inputs,
                                    discrete_outputs, dinputs, doutputs)
            else:
                fwd_grad = fwd_func(comp, inputs, outputs, resids, dinputs, doutputs)
            dresids._data[:] = rand_outputs
            if discrete:
                rev_grad = rev_func(comp, inputs, outputs, resids, discrete_inputs,
                                    discrete_outputs, dresids)
            else:
    ...
    for array in arrays:
        for idx in range(array.size):
            ...
            if mode == 'fwd':
                if explicit:
                    if discrete:
                        grad = deriv_func(comp, inputs, outputs, discrete_inputs, discrete_outputs,
                                          dinputs)
                    else:
                        grad = deriv_func(comp, inputs, outputs, dinputs)
                else:
                    if discrete:
                        grad = deriv_func(comp, inputs, outputs, resids, discrete_inputs,
                                          discrete_outputs, dinputs, doutputs)
                    else:
