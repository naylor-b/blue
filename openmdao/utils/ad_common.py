...
def _ad(prob, options):
    ...
    for comp in it:
        ...
        if isinstance(comp, ExplicitComponent):
            summ['analytic'] = overrides_method('compute_partials', comp, ExplicitComponent)
        else:
            summ['analytic'] = overrides_method('linearize', comp, ImplicitComponent)
        summ['analytic'] |= comp.matrix_free
        if summ['analytic']:
            summ['status'] = 'OK'
    ...
    toptemplate = \
        "{n:<{cwid}}{typ:<10}{fdiff:<{dwid}}{rdiff:<{dwid}}{dot:<{dwid}}{iosz:<12}{pref:^10}"\
        "{analytic:^10}{status:^8}"
    template = \
        "{n:<{cwid}}{typ:<10}{fdiff:<{dwid}.4}{rdiff:<{dwid}.4}{dot:<{dwid}.4}{iosz:<12}{pref:^10}"\
        "{analytic:^10}{status:^8}"
    print(toptemplate.format(n='Class', typ='Type', fdiff='Max Diff (fwd)',
                             rdiff='Max Diff (rev)', dot='Dotprod Test', pref='Preferred',
                             analytic='Analytic', status='Status',
                             cwid=max_cname, dwid=max_diff, iosz='(I/O) Size'))
    print('--------- both AD derivs OK ------------')
    for cname, s in sorted(summary.items(),
        ...
        if fwdran and revran and fwdmax <= tol and revmax <= tol:
            ...
            line = template.format(n=cname, typ=typ, fdiff=fwdmax, rdiff=revmax, dot=dptest,
                                   ...
                                   analytic=str(s['analytic']), status=s['status'])
    ...
    if fwdgood:
        print('--------- fwd AD derivs OK ------------')
        for b in fwdgood:
            print(b)

    if revgood:
        print('--------- rev AD derivs OK ------------')
        for b in revgood:
            print(b)

    if bad:
        print('--------- both AD derivs BAD ------------')
        for b in bad:
            print(b)
