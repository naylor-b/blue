...
class AssembledJacobian(Jacobian):
    ...
    def _init_view(self, system):
        ...
        in_offset = {}
        get_inp = system._inputs._abs_get_val
        start = 0
        for n in system._var_abs2meta['input']:
            if n not in conns:  # connected to output outside of this system
                in_offset[n] = start
            start += get_inp(n).size
    ...
    def _apply(self, system, d_inputs, d_outputs, d_residuals, mode):
        ...
        with system._unscaled_context(outputs=[d_outputs], residuals=[d_residuals]):
            ...
            if mode == 'fwd':
                ...
                if d_outputs._names:
                    d_outputs.iadd(int_mtx._prod(dresids, mode))
                if do_mask:
