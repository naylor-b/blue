...
class LinesearchSolver(NonlinearSolver):
    ...
    def _enforce_bounds(self, step, alpha):
        ...
        """
        system = self._system()
        method = self.options['bound_enforcement']

        if self.options['print_bound_enforce']:
            _print_violations(system._outputs, system._lower_bounds, system._upper_bounds)

        if method == 'vector':
            _enforce_bounds_vector(system._outputs, step, alpha,
                                   system._lower_bounds, system._upper_bounds)
        elif method == 'scalar':
            _enforce_bounds_scalar(system._outputs, step, alpha,
                                   system._lower_bounds, system._upper_bounds)
        elif method == 'wall':
            _enforce_bounds_wall(system._outputs, step, alpha,
                                 system._lower_bounds, system._upper_bounds)
...
def _enforce_bounds_vector(u, du, alpha, lower_bounds, upper_bounds):
    ...
    ----------
    u : <Vector>
        The backtracking is applied to this vector in-place.
    du : <Vector>
        Newton step; the backtracking is applied to this vector in-place.
    alpha : float
        step size.
